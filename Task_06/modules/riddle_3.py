...
def func_riddle_list (count_riddle):
    ...
    while riddle_list and count_riddle>0:
...
def game_start (count_riddle, count_lim):
    a = 0
    riddle_dict_gen = func_riddle_list(count_riddle)
    while True:
        try:
...
def show_results ():
    ...
    for riddle, count in _results.items():
        riddle += ": "
        result.append(f"{riddle:<100} отгадана с {count} попытки")
    return "\n".join(result)