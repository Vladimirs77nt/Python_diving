...
def info_dir_and_file (dir_path, result_list={}, size=0):
    ...
    for i in data_dir:
        ...
        if os.path.isfile(_file):
            list_file.append (i)    # сюда складываем файлы
        else:
            list_dir.append (i)     # сюда складываем папки
    
    # обход папок
    for _dir in list_dir:
        result_list, _size = info_dir_and_file (dir_path + "/" + _dir, result_list)
        size += _size

    # обход файлов внутри папки
    temp_list = []
    for i_file in list_file:
        i_size = os.path.getsize(dir_path + "/" + i_file)
        file_info = f" - файл {i_file} ({dir_path}, размер {i_size} байт)"
        temp_list.append (file_info)
        size += i_size

    dir_info = f"> Папка {dir_path} ({size})"
...
# ---------- ЗАПУСК ПРОГРАММЫ -------------
dir_path = "E:/codes/DZ_06"    # <-- рабочая папка для работы
os.chdir(dir_path)
print (dir_path)

result, *_ = info_dir_and_file (dir_path)
print ("--РЕЗУЛЬТАТ--")
for key, temp_list in result.items():
    print (key)
    for file in temp_list:
        print (file)

dir_path = "E:/codes/DZ_08"    # <-- рабочая папка для записи
file_name = "result_scan_2"
os.chdir(dir_path)
